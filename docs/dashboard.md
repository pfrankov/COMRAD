...
- **Storage**: artifact kind, sha256, torrent info hash, magnet link, current seeders, verification state, cached nodes, model usage, and guarded deletion for unused artifacts. Artifacts are public-distribution artifacts: Workers try public BitTorrent delivery first when available, then fall back to authenticated Manager HTTP.
...
- **Updates**: Worker software updates, pending rollout records, impact preview, delivery method, and failures. Model edits do not use updates.
- **Settings**: admin token storage reachable before protected state loads, component purpose map, safety posture, links to operational settings, the built-in API reference, a read-only runtime adapter summary, and a highlighted runtime YAML config from `/api/admin/config.yaml`, including Worker P2P runtime availability and effective networking settings.
