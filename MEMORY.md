...
- Dashboard WebSocket health is exported in Prometheus metrics, including active clients, connects, broadcasts, dropped updates, write failures, last snapshot bytes, and last broadcast subscribers.
...
- Capacity policies can opt into auto-balance with min/max downloaded and ready counts. The planner derives effective demand from queued, running, and recent requests, then plans globally across Worker RAM/disk so scarce large models are placed before smaller extras.
- Deleting a model removes its profile, capacity policy, and assignments, then queues guarded Worker cache eviction for artifacts that are no longer desired or active. **Nodes** can also remove a stale cached artifact from one selected online Worker.
